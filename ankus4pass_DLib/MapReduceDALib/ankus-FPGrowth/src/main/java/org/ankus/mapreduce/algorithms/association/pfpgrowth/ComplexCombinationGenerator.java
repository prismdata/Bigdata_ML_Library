...
public class ComplexCombinationGenerator<T> extends
	...
	public static <T> String convert2String(ICombinatoricsVector<ICombinatoricsVector<T>> vector) {
		StringBuilder sb = new StringBuilder();

		for (int i = 0; i < vector.getSize(); i++)
		{
			System.out.println(vector.getValue(i).getVector().toString());
	...
	public static <T> List convert2List(ICombinatoricsVector<ICombinatoricsVector<T>> vector) {
		
		List<String> vector_list = new ArrayList<String>(); 
		
		for (int i = 0; i < vector.getSize(); i++)
		{
			vector_list.add(vector.getValue(i).getVector().toString());
		}
		return vector_list;
	}
