...
public abstract class Generator<T> implements
	 ...
	 */
	public static int Rule_length;
	public List<ICombinatoricsVector<T>> generateFilteredObjects(IFilter<ICombinatoricsVector<T>> filter)
			{
				List<ICombinatoricsVector<T>> list = new ArrayList<ICombinatoricsVector<T>>(1000);
				long index = 0;
				for (ICombinatoricsVector<T> vector : this)
				{
					
					if( Rule_length >= vector.getSize())
					{
						System.out.println("Combine Index:" + index);
