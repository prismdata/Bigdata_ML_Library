...
public class NominalStatsDriver extends Configured implements Tool {
	    ...
	    */
		...
		public int run(String[] args) throws Exception
		{
	        ...
	        FileSystem.get(conf).delete(new Path(conf.get(ArgumentsConstants.OUTPUT_PATH) + "/result"));
//	        범주 데이터의 발생 빈도를 계산한다.	        
...
//	        최종 결과물을 파일로 기록한다.
			...
			System.out.println("Nominal Statistics Finished TIME(ms) : " + lTime/1000000.0 + "(ms)");
			System.out.println("Nominal Statistics Finished TIME(s) : " + (lTime/1000000.0)/1000 + "(s)");
...
//			임시 파일을 삭제한다.
	    ...
	    * @auth Moonie
	    *
	    * @parameter Configuration conf  Hadoop 환경 설정 변수
	    * @parameter String inputPath    입력 경로
	    * @parameter long dataCnt        전체 개체 갯수
	    * @parameter String outputFile   출력 경로
	    *             
	    * @return
	    * 
	    */
	    ...
	    * @param args :NominalStatsDriver를 구동하기 위한 인자.
	    * @throws Exception
	    */
		...
		* @parameter
		*      job : job 구성 정보 변수.
		*      conf : Hadoop 환경 설정 변수.
		*      outputPathStr : job 결과 출력 경로.
		*/
