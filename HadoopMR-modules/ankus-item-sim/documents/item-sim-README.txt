추천 알고리즘의 아이템간 유사도 모델 생성 

수행 방법:
hadoop jar ankus-core2-item-sim-1.1.0.jar \
ItemBasedSimilarity \
-input [입력 파일 또는 입력 폴더] \
-output [출력 폴더 경로] \
-delimiter [속성 구분자(movielens 데이터의 경우 ::로 구분함] \
-algorithmOption [유사도 계산 방법 {uclidean, cosine, pearson}] \
-commonCount [두 사용자(아이템)간 공통 평가 점수의 개수를 지정 ] \
-uidIndex [사용자 컬럼 위치] \
-iidIndex [아이템 컬럼 위치] \
-ratingIndex [평점 컬럼 위치]
-commonCount : 유사도 계산의 정확성을 유지하기 위해, 두 사용자(아이템)간 공통 평가 점수의 개수를 지정,지정하지 않는 경우 기본 10 ]
 **/item_sim_result/part-r-00000 아이템간 유사도 정보를 생성함.
 
수행 예제:
hadoop jar ankus-item-sim/target/ankus-core2-item-sim-1.1.0.jar \
ItemBasedSimilarity \
-input /data/item-sim.txt \
-output /resut/item_sim_result \
-delimiter :: \
-algorithmOption uclidean \
-commonCount 10 \
-uidIndex 0 \
-iidIndex 1 \
-ratingIndex 2

*입력 예제
.....
U5329::3524::4::961005226
U5329::3526::5::961011305
U5329::3528::4::961007575
U5329::2580::4::960916782
U5329::318::5::961002454
U5329::3535::4::968790599
U5329::3538::5::987486696
U5329::324::4::961456452
U5329::2598::3::960916999
U5329::2599::4::960916545
U5329::3543::5::960916528
U5329::3549::4::960837585
U5329::1946::5::960838364
U5329::11::4::961008571
U5329::337::5::960839374
U5329::17::5::960838561
U5329::3551::5::1004073143
U5329::3556::3::987486791
.....
*출력 예제
**/item_sim_result/part-r-00000
1000::1183::4.123
1000::1193::6.708
1000::1196::8.0
1000::1197::7.0
1000::1198::7.937
1000::1200::7.141
1000::1208::6.324
1000::1210::8.124
1000::1213::8.485
1000::1214::7.549
1000::1215::7.280
1000::1219::5.916
1000::1221::8.0
1000::1222::7.141
1000::1240::7.0
