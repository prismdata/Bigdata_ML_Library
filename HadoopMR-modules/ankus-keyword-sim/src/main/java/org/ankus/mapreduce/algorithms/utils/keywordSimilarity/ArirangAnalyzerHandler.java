...
public class ArirangAnalyzerHandler {
    ...
    public ArirangAnalyzerHandler(boolean keyword , Configuration conf, String path)
    {
    	...
    	if(keyword == true)
    	{
	    	try
	    	{
	    		...
	    		Path[] targetPathArr = DistributedCache.getLocalCacheFiles(conf);//Get All Term
    ...
    */
    ...
    private List <String> guideListWord_restrict(String source) throws MorphException {
        MorphAnalyzer maAnal = new MorphAnalyzer(); // 형태소 분석기 
     
        StringTokenizer stok = new StringTokenizer(source, " "); // 쿼리문을 뛰어쓰기 기준으로 토큰화
        List <String> SentenceWordIndex = new ArrayList<String>();
        while (stok.hasMoreTokens()) 
        {
    ...
    public List<String> getListKeyWord(String documents)
    {
...
//    			Set <String> docWord = this.guideWord_restrict(documents);
    		...
    		{ 
    			List <String> docWord = this.guideListWord_restrict(documents);    	    	
    	    	if(docWord.size() > 0)
    			{
    		        for (String str_Token : docWord) 
    		        {
    ...
    public Set<String> getSetKeyWord(String documents)
    {
    	Set<String> MDFS_KEYWORD = null;
    	try
    	{
    		documents = documents.replaceAll("\\[|\\]|\\(|\\)|\\▶|\\ⓒ|\\‘|\\,|\\’|\\.|\\-", " ");
    		String match = "[^\uAC00-\uD7A3xfe0-9a-zA-Z\\s]";
    		...
    		boolean BExist = false;
    		MDFS_KEYWORD = new HashSet<String>();
    		if(use_keyword == true )
    		{    			
    			//중요 단어 부분매칭 사용 1차 검출.
    			Iterator<String> keys = importantkey.keySet().iterator();
